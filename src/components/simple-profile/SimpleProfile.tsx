  ...
  return (
    <Link to={`/author/${userData.user_id}`} className="inline-block">
      <div className="flex items-center gap-1">
        <div
          className={`flex justify-center overflow-auto rounded-full border border-none`}
        >
          {userData.user_image ? (
            <img
              src={userData.user_image}
              alt="프로필사진"
              className={`object-cover ${imageSizeStyles[imageSize]}`}
            />
          ) : (
            <CgProfile size={20} />
          )}
        ...
        <div
          className={`text-gray-700 font-semibold ${textSizeStyles[textSize]}`}
        >
